...
int FORK_SORT()
{
    int child_pid[ MAX_PROCESS ] , begin = 0 , end = 0 , i , j , index , status;
    char fullname[ NAME_LENGTH ] = "merge_", *str;
    str = fullname + 6;
    ...
    for( i=0 ; end!=TXT_LENGTH ; begin+=CHUNK_LENGTH )
    {
        ...
        index = i%MAX_PROCESS;
        while( (child_pid[ index ] = fork()) < 0 )
            wait( &status );

        if( child_pid[ index ] == 0 )
        {
            MP3_SORT( begin , end , fullname );
            _exit(1);
        }
...
int FORK_MERGE()
{
    ...
    char *files[MERGE_DEGREE+1], *str[MERGE_DEGREE+1];
    for( x=0 ; x<=MERGE_DEGREE ; x++ )
    {
    ...
    for( step=0 ; ; step++ , chunk_size*=MERGE_DEGREE )
    {
        for( i=0 , tar_beg=0 ; ; tar_beg+=chunk_size*MERGE_DEGREE )
        {
            ...
            for( file_count=1 , beg=end=tar_beg ;
...
int main ( int argc , char *argv[] )
{
    if( argc == 4 )
    {
        CHILD_MAX = sysconf( _SC_CHILD_MAX )-6;
